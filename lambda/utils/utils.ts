...
export const verifyToken = async (token: string): Promise<{ decoded: Partial<IUser>; authenticated: boolean }> => {
    try {
        ...
        return { decoded, authenticated: !!decoded._id };
    } catch (error) {
        console.error("Token verification failed: ", error);
        return { decoded: {}, authenticated: false }; // Return a consistent object structure
    }
};
...
export const ErrorReponse = (statusCode?: number, error?: any, message?: string) => {
    return {
